...
Wire format (all offsets little-endian, counted from #C000 of the first
pack bank; the blob itself starts at bank_offset in that bank):

  +0   u16 frame_count
  +2   u16 loop_frame           first frame of the loop block
  +4   u16 loop_offset          interlaced loop-block offset
  +6   u16 data_offset          first interlaced byte, bank_offset + 8
  +8   interlaced packed bytes (prefix block, then loop block)

The runtime maps the blob across consecutive 16 KiB banks at #C000. With
--pack-in-assets the blob is appended to the last asset bank when it fits
there; otherwise it starts at offset 0 of bank 0. A
...
KERNEL_NAMES = ["countdown", "dds_duty", "dds_edge", "sample", "wavetable", "dds_sample"]
...
class AssetBankSet:
    ...
    def __init__(self, samples, wavetables):
        ...
        self.bank_count = 0
        self.pack_bank, self.pack_offset = 0, 0
        self.image = b""
    ...
    def build(self):
        ...
        self.image = b"".join(images[:self.bank_count])
        return self.image
    ...
    def pack_tail(self):
        """(hardware bank, offset) of the free tail of the last asset bank."""
        index = self.bank_count - 1
        return (4, 6)[index], self.bank_used[index]

    def append_pack(self, blob: bytes):
        """Append a blob packed with bank_offset = the tail offset to the last
        asset bank image. Returns the new asset image."""
        bank, offset = self.pack_tail()
        if offset + len(blob) > ASSET_BANK_SIZE:
            raise ValueError("pack does not fit the asset bank tail")
        self.bank_used[self.bank_count - 1] = offset + len(blob)
        self.pack_bank, self.pack_offset = bank, offset
        image = bytearray(self.image)
        start = (self.bank_count - 1) * ASSET_BANK_SIZE + offset
        image[start:start + len(blob)] = blob
        self.image = bytes(image)
        return self.image

    def write_dir_inc(self, path, pack_bytes=0, kernels=0):
        """Write an equ-only bank summary for the public wrapper: asset bank
        sizes, where the pack lives (bank 0 unless append_pack ran), asset
        counts and the kernel mask the track uses."""
        if not self.bank_count:
            raise RuntimeError("build asset banks before writing the directory")
        ...
        lines += [f"PackBank equ {self.pack_bank}",
                  f"PackOffset equ {self.pack_offset}",
                  f"PackBytes equ {pack_bytes}",
                  f"SampleCount equ {len(self.samples)}",
                  f"WavetableCount equ {len(self.wavetables)}",
                  f"KernelMask equ {kernels}"]
        Path(path).write_text("\n".join(lines) + "\n")
...
def kernel_mask(psg: bytes, events: bytes) -> int:
    """Bit i set = kernel id i has a START or MODIFY entry in the track."""
    lanes, entry_mask = build_lanes(psg, events)
    return sum({1 << ((lanes[14][f] >> 4) & 7) for f, has in enumerate(entry_mask) if has})
...
def unpack_interlaced_block(blob: bytes, ofs: int, frame_count: int, base: int = 0):
    """Reference-decode one interlaced block. ofs is a bank-relative position;
    blob[0] sits at position base. Return (lanes, end position)."""
...
def _pack_layout(lanes, loop_frame: int, bank_offset: int = 0):
    """Return (blob, lane sizes, prefix bytes, data_offset) for 20 equal-length
    lanes. The blob starts at bank_offset of its first bank; header offsets
    count from the bank start, so data_offset is bank_offset + HEADER_SIZE.
    Frame-start bank padding (see interlace_block) lands inside the
    prefix/loop bytes instead."""
    assert len(lanes) == NLANES
    n = len(lanes[0])
    assert all(len(lane) == n for lane in lanes)
    if n == 0:
    ...
    if not 0 <= bank_offset <= BANK_SIZE - 256 - HEADER_SIZE:
        raise ValueError(f"bank_offset {bank_offset} outside the first bank")
    data_off = bank_offset + HEADER_SIZE
...
def pack(lanes, loop_frame: int, bank_offset: int = 0) -> bytes:
    """Pack 20 independent lanes into one interlaced wire blob."""
    return _pack_layout(lanes, loop_frame, bank_offset)[0]


def unpack(blob: bytes, bank_offset: int = 0):
    """Reference file decoder for a blob that starts at bank_offset of its
    first bank. Returns (lanes, loop_frame)."""
    n, loop_frame, loop_ofs, data_off = struct.unpack_from("<HHHH", blob, 0)
    end_pos = bank_offset + len(blob)
    if not bank_offset + HEADER_SIZE <= data_off <= loop_ofs <= end_pos:
        raise ValueError(f"data/loop offsets {data_off}/{loop_ofs} outside packed blob")
    if loop_frame:
        prefix, end = unpack_interlaced_block(blob, data_off, loop_frame, bank_offset)
        if end != loop_ofs:
    ...
    loop, end = unpack_interlaced_block(blob, loop_ofs, n - loop_frame, bank_offset)
    if end != end_pos:
        raise ValueError(f"packed block leaves {end_pos - end} trailing bytes")
    return [prefix[k] + loop[k] for k in range(NLANES)], loop_frame


# --------------------------------------------------------------------------
def pack_streams(psg: bytes, events: bytes, loop_frame: int = 0, bank_offset: int = 0):
    ...
    bank_offset is where the blob will sit in its first bank."""
    lanes, entry_mask = build_lanes(psg, events)
    source_lanes = lanes
    ...
    for k in range(16, NLANES):
        ...
        if cost(rept) < cost(zero):
            ...
            final.append(zero)
            fills.append("zero")
    blob, lane_sizes, prefix_bytes, data_off = _pack_layout(final, loop_frame, bank_offset)

    # verify: reference depack must reproduce every lane byte
    out_lanes, out_loop = unpack(blob, bank_offset)
...
def main():
    ...
    ap.add_argument("--pack-in-assets", action="store_true",
                     help="append the pack to the last asset bank when it fits "
                          "(the wrapper reads PackBank/PackOffset from _dir.inc)")
    args = ap.parse_args()
    ...
    try:
        ...
        blob = None
        if args.pack_in_assets:
            _bank, offset = asset_banks.pack_tail()
            blob, st = pack_streams(psg, events, args.loop_frame, offset)
            if offset + len(blob) <= ASSET_BANK_SIZE:
                assets_blob = asset_banks.append_pack(blob)
            else:
                blob = None
        if blob is None:
    ...
    kernels = kernel_mask(psg, events)
    asset_banks.write_dir_inc(dir_inc_path, len(blob), kernels)
    ...
    if asset_banks.pack_bank:
        print(f"  pack placed in asset bank {asset_banks.pack_bank} "
              f"at offset {asset_banks.pack_offset}")
    else:
    ...
    used = ", ".join(KERNEL_NAMES[k] for k in range(6) if kernels >> k & 1) or "none"
    print(f"  kernels used: {used} (mask {kernels})")
    usage = ", ".join(
        f"bank {bank}={used} B"
